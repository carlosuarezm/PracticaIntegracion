...
class CartManager {
    ...
    async createCart(newCart) {
        ...
        if (this.validateProducts(newCart.products)) {
            ...
            console.log(`This Cart has been added: `, newCart)
    ...
    async getCarts() {
        try {
            ...
            data === '' ? data = this.carts : this.carts = JSON.parse(data) // Coversion de JSON a obejto 
            return this.carts
        } catch (error) {
            console.log('The read has failed'); //Mensaje de error en caso de falla de escritura    
        }
    }
    ...
    async saveCarts() {
        ...
        try {
            ...
            console.log(`The wite has been completed in ${this.path}`); //Mensaje de confirmacion

        } catch (error) {
            console.log('The write has failed', err); //Mensaje de error en caso de falla de escritura
        }
    }
    ...
    async getCartById(id) {
        await this.getCarts()  //Actualiza el array de productos desde el contenido del archivo en la ruta "path"
        let cart = this.carts.find((c) => c.id == id) //Busca el objeto que coincida con el id recibido
